...
* 1단계: 프로젝트 기획 및 아키텍처 설계
  ...
  * 작업 내용:
    * 주요 기능 정의:
      * 콘텐츠 스트리밍, 비행 정보, 음식 주문 등 IFE 기능 정리.
    * 아키텍처 설계:
      * 백엔드-프론트엔드 통합 구조, 데이터베이스 설계, 스트리밍 서버 위치 결정.
    * UI/UX 와이어프레임 정리:
      * Figma에서 화면별 기능 흐름 설계.
  * 다음 단계로 넘어가는 조건:
    * UI 설계(와이어프레임) 완성.
...
* 탑승객 기능:
  * 개인 기기와의 미러링 기능:
    * 탑승객이 자신의 스마트폰 또는 태블릿 화면을 좌석 디스플레이에 미러링.
    * 목적:
    * 개인 기기에서 스트리밍하거나 문서 작업을 이어서 진행 가능.
    * 기술적 요구사항:
    * Miracast, AirPlay, Google Cast 지원.
  * 소셜 공유 기능:
    * 콘텐츠를 소셜 미디어(페이스북, 트위터 등)에 추천 또는 공유.
    * 목적:
    * 엔터테인먼트를 통한 항공사 브랜드 홍보.
    * 기술적 요구사항:
    * 소셜 미디어 API와의 연동.
  * 플라이트 게임:
    * 간단한 게임 제공(Tetris, Trivia 등).
    * 목적:
    * 엔터테인먼트 확장 및 어린이 고객 만족.
    * 기술적 요구사항:
    * 오프라인 HTML5 기반 게임 지원.
* 항공사 기능:
  * 비행 데이터 실시간 분석:
    * 항공기의 실시간 운항 정보 분석 및 레포트 생성.
    * 목적:
    * 비행 경로 최적화 및 안전 운항 지원.
    * 기술적 요구사항:
    * 항공기 센서 데이터와의 통합.
  * 광고 및 프로모션:
    * 콘텐츠 사이에 항공사 광고 또는 프로모션 삽입.
    * 목적:
    * 추가 매출 창출.
    * 기술적 요구사항:
    * CMS에서 광고 스케줄 관리.
  * 비상 알림 전송:
    * 항공사 또는 승무원이 모든 디스플레이에 비상 알림 전송.
    * 목적:
    * 비상 상황 대응.
    * 기술적 요구사항:
    * API를 통한 알림 브로드캐스트.
  * 콘텐츠 사용자 평가 분석:
    * 탑승객이 콘텐츠에 평점과 리뷰를 남길 수 있도록 함.
    * 목적:
    * 고객 선호도를 기반으로 콘텐츠를 개선.
    * 기술적 요구사항:
